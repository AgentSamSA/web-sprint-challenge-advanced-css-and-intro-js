...
*/
function getArtistByIndex(artistArray, artistIndex) {
  return `the artist at index ${artistIndex} is ${artistArray[artistIndex].name}`;
}
...
function get20s(artistArray) {
  ...
  for (let i = 0; i < artistArray.length; i++) {
    ...
    if (yearsLived[0] >= 1900) {
      if (yearsLived[2] < 2000) {
        artistsIn20s.push(artistArray[i].name);
      }
    }
  }
  return artistsIn20s;
...
*/
function removeArtist(artistArray, artistIndex) {
...
const newArtist = {
  ...
  bio: "Samuel Lee is a current Web Dev student at Lambda School, part of the 37th cohort. He enjoys building things, and even drew pictures when he was young.",
}
...
function lotsOfArt(artistArray) {
  ...
  for (let i = 0; i < artistArray.length; i++) {
    if (artistArray[i].paintings > 100) {
      prolificArtists.push(artistArray[i].name);
    }
  }
